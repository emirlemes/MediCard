...
import { closeDatabase, createDatabaseBackup, validateDatabaseBackup } from './database'
...
function formatReportDateTime(value: string) {
  ...
  return `${formatReportDate(value)} ${hours}:${minutes}`
}
...
function reportHtml(report: ReportInput) {
  ...
  return `<!doctype html><html><head><meta charset="utf-8"><style>
    ...
    body { margin: 0; padding: 18mm; color: #243b39; font: 11pt Arial, sans-serif; line-height: 1.45; }
    ...
    <h2>Podaci pacijenta</h2><div class="patient"><span><span class="label">Ime i prezime</span>${escapeHtml(patient.firstName)} ${escapeHtml(patient.lastName)}</span><span><span class="label">Broj kartona</span>${escapeHtml(patient.recordNumber)}</span><span><span class="label">Datum rođenja</span>${escapeHtml(formatReportDate(patient.dateOfBirth))}</span><span><span class="label">JMBG</span>${escapeHtml(patient.nationalId || 'Nije uneseno')}</span><span><span class="label">Telefon</span>${escapeHtml(patient.phone || 'Nije uneseno')}</span><span><span class="label">Adresa</span>${escapeHtml(patient.address || 'Nije uneseno')}</span></div>
    <h2>Pregled</h2><div class="meta"><span><span class="label">Datum pregleda</span>${escapeHtml(formatReportDateTime(examination.examinationAt))}</span><span><span class="label">Doktor</span>${escapeHtml(examination.doctorName)}</span><span><span class="label">Dijagnoza</span>${escapeHtml(examination.diagnosis || 'Nije unesena')}</span></div>
...
async function loadReport(database: PrismaClient, examinationId: string): Promise<ReportInput> {
  const examination = await database.examination.findUnique({
    where: { id: examinationId },
    include: { patient: true, department: true, therapy: { include: { items: true } } },
  })
  ...
  return {
    patient: {
      ...
      recordNumber: examination.patient.recordNumber,
      dateOfBirth: examination.patient.dateOfBirth?.toISOString() ?? null,
      nationalId: examination.patient.nationalId,
      phone: examination.patient.phone,
      address: examination.patient.address,
    },
    examination: {
      examinationAt: examination.examinationAt.toISOString(),
      ...
      therapy: examination.therapy ? { items: examination.therapy.items.map((item) => ({ medicineName: item.medicineName, dosage: item.dosage, administrationRoute: item.administrationRoute, duration: item.duration, note: item.note })) } : null,
    },
    settings: settings ? { clinicName: settings.clinicName, address: settings.address, phone: settings.phone, email: settings.email, doctorName: settings.doctorName } : null,
  }
}
...
export function registerIpc(database: PrismaClient, databasePath: string) {
  ...
  ipcMain.handle('departments:create', async (_event, input: { name: string }) => {
    const name = input.name.trim()
    if (!name) throw new Error('Naziv odjela je obavezan.')
    const existingDepartments = await database.department.findMany({ select: { name: true } })
    if (existingDepartments.some((department) => department.name.trim().toLocaleLowerCase('bs-BA') === name.toLocaleLowerCase('bs-BA'))) {
      throw new Error('Odjel sa tim nazivom već postoji.')
    }
    return database.department.create({ data: { name } })
  })

  ipcMain.handle('departments:update', async (_event, input: { id: string; name: string }) => {
    const name = input.name.trim()
    if (!name) throw new Error('Naziv odjela je obavezan.')
    const existingDepartments = await database.department.findMany({ where: { NOT: { id: input.id } }, select: { name: true } })
    if (existingDepartments.some((department) => department.name.trim().toLocaleLowerCase('bs-BA') === name.toLocaleLowerCase('bs-BA'))) {
      throw new Error('Odjel sa tim nazivom već postoji.')
    }
  ...
  ipcMain.handle('departments:set-active', async (_event, input: { id: string; active: boolean }) => {
    if (!input.active) {
      const settings = await database.clinicSettings.findUnique({ where: { id: 1 }, select: { departmentId: true } })
      if (settings?.departmentId === input.id) throw new Error('Ne možete deaktivirati trenutno aktivni odjel.')
    }
    return database.department.update({ where: { id: input.id }, data: { active: input.active } })
  })

  ipcMain.handle('settings:get', async () => database.clinicSettings.findUnique({ where: { id: 1 } }))

  ipcMain.handle('settings:update', async (_event, input: { clinicName: string; address: string; phone: string; email: string; doctorName: string; departmentId?: string }) => {
    if (input.departmentId) {
      const department = await database.department.findUnique({ where: { id: input.departmentId }, select: { active: true } })
      if (!department) throw new Error('Odabrani odjel ne postoji.')
      if (!department.active) throw new Error('Odabrani odjel nije aktivan.')
    }
    return database.clinicSettings.update({ where: { id: 1 }, data: input })
  })
  ...
  ipcMain.handle('patients:create', async (_event, input: { firstName: string; lastName: string; recordNumber: string; phone?: string; dateOfBirth?: string; nationalId?: string; address?: string; note?: string }) => {
    const firstName = input.firstName.trim()
    const lastName = input.lastName.trim()
    const recordNumber = input.recordNumber.trim()
    if (!firstName || !lastName || !recordNumber) throw new Error('Ime, prezime i broj kartona su obavezni.')
    const dateOfBirth = input.dateOfBirth ? new Date(input.dateOfBirth) : undefined
    if (dateOfBirth && Number.isNaN(dateOfBirth.getTime())) throw new Error('Datum rođenja nije ispravan.')
    return database.patient.create({
      data: { firstName, lastName, recordNumber, phone: input.phone?.trim() || null, dateOfBirth, nationalId: input.nationalId?.trim() || null, address: input.address?.trim() || null, note: input.note?.trim() || null },
    })
  })
  ...
  ipcMain.handle('patients:export-csv', async (event, patients: { firstName: string; lastName: string; recordNumber: string; dateOfBirth: string | null; nationalId: string | null; phone: string | null; address: string | null; note: string | null }[]) => {
    ...
    const rows = patients.map((patient) => [patient.firstName, patient.lastName, patient.recordNumber, patient.dateOfBirth ? formatReportDate(patient.dateOfBirth) : '', patient.nationalId, patient.phone, patient.address, patient.note].map(escapeCsv).join(';'))
    await fs.writeFile(result.filePath, `\ufeff${header.map(escapeCsv).join(';')}\n${rows.join('\n')}`, 'utf8')
    return { saved: true, filePath: result.filePath }
  })

  ipcMain.handle('examinations:create', async (_event, input: { patientId: string; departmentId: string; examinationAt: string; doctorName: string; diagnosis?: string; findings?: string; recommendation?: string; therapyItems: { medicineName: string; dosage?: string; administrationRoute?: string; duration?: string; note?: string }[] }) => {
    const [patient, department] = await Promise.all([
      database.patient.findUnique({ where: { id: input.patientId }, select: { lockedAt: true } }),
      database.department.findUnique({ where: { id: input.departmentId }, select: { active: true } }),
    ])
    if (!patient) throw new Error('Pacijent nije pronađen.')
    if (patient.lockedAt) throw new Error('Karton je zaključan nakon štampanja.')
    if (!department?.active) throw new Error('Odabrani odjel nije aktivan.')
    const examinationAt = new Date(input.examinationAt)
    if (Number.isNaN(examinationAt.getTime())) throw new Error('Datum pregleda nije ispravan.')
    const doctorName = input.doctorName.trim()
    if (!doctorName) throw new Error('Doktor je obavezan.')
  ...
  ipcMain.handle('examinations:update', async (_event, input: { id: string; diagnosis?: string; findings?: string; recommendation?: string; therapyItems?: { medicineName: string; dosage?: string; administrationRoute?: string; duration?: string; note?: string }[] }) => {
    ...
    return database.$transaction(async (transaction) => {
      await transaction.examination.update({ where: { id: input.id }, data: { diagnosis: input.diagnosis?.trim() || null, findings: input.findings?.trim() || null, recommendation: input.recommendation?.trim() || null } })
  ...
  ipcMain.handle('examinations:export-csv', async (event, input: { patientName: string; recordNumber: string; examinations: { examinationAt: string; doctorName: string; diagnosis: string | null; findings: string | null; recommendation: string | null; department: { name: string }; therapy: { items: { medicineName: string; dosage: string | null; duration: string | null }[] } | null }[] }) => {
    ...
    const rows = input.examinations.map((examination) => [input.patientName, input.recordNumber, formatReportDateTime(examination.examinationAt), examination.department.name, examination.doctorName, examination.diagnosis, examination.findings, examination.recommendation, examination.therapy?.items.map((item) => [item.medicineName, item.dosage, item.duration].filter(Boolean).join(' - ')).join(', ')].map(escapeCsv).join(';'))
    await fs.writeFile(result.filePath, `\ufeff${header.map(escapeCsv).join(';')}\n${rows.join('\n')}`, 'utf8')
    return { saved: true, filePath: result.filePath }
  })

  ipcMain.handle('reports:pdf', async (event, input: { examinationId: string }) => {
    const report = await loadReport(database, input.examinationId)
    const owner = BrowserWindow.fromWebContents(event.sender)
  ...
  ipcMain.handle('reports:preview', async (_event, input: { examinationId: string }) => {
    const report = await loadReport(database, input.examinationId)
    const reportWindow = await createReportWindow(reportHtml(report))
    reportWindow.setTitle(`Pregled nalaza - ${report.patient.firstName} ${report.patient.lastName}`)
    reportWindow.show()
    return { opened: true }
  })

  ipcMain.handle('reports:print', async (_event, input: { examinationId: string }) => {
    const report = await loadReport(database, input.examinationId)
    const reportWindow = await createReportWindow(reportHtml(report))
    try {
      const printers = await reportWindow.webContents.getPrintersAsync()
      if (printers.length === 0) return { printed: false, reason: 'Nema dostupnih printera.' }
      ...
      await database.patient.update({ where: { id: report.patient.id }, data: { lockedAt } })
      return { printed: true, lockedAt: lockedAt.toISOString() }
    } finally {
      if (!reportWindow.isDestroyed()) reportWindow.close()
    }
  })
  ...
  ipcMain.handle('backup:create', async (event) => {
    ...
    if (result.canceled || !result.filePath) return { saved: false }
    await createDatabaseBackup(databasePath, result.filePath)
    return { saved: true, filePath: result.filePath }
  })
  ...
  ipcMain.handle('backup:restore', async (event) => {
    ...
    validateDatabaseBackup(backupPath)
    const temporaryPath = `${databasePath}.restore.tmp`
    const previousPath = `${databasePath}.before-restore`
    await fs.copyFile(backupPath, temporaryPath)
    validateDatabaseBackup(temporaryPath)
    await closeDatabase()
    try {
